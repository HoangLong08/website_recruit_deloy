...
import { getListRecruitAction, blockOrUnblockAction } from 'store/actions/admin';
...
function ManageListRecruit() {
  const dispatch = useDispatch();
  const listRecruit = useSelector(state => state.adminSlice.listRecruit);

  const [isModalVisible, setIsModalVisible] = useState('');

  useEffect(() => {
    dispatch(getListRecruitAction());
  }, [dispatch]);
  ...
  const renderDataSource = () => {
    return listRecruit.data?.map(item => {
      return {
        key: nanoid(),
        name: item.user_name,
  ...
  return (
    <div className="wrapper-content-inner-client customize-scroll">
      <div className="content-inner-client-head">
        <p className="content-inner-client-head-title">Danh sách nhà tuyển dụng</p>
      </div>
      <div className="content-inner-client-bottom">
        <TableComponent data={renderDataSource()} tableHead={columns} loading={listRecruit.load} />
      </div>
...
export default ManageListRecruit;
